...
async listParticipants(filter: ParticipantFilter) : Promise<Participant[]> {
    return await TAURI_INVOKE("list_participants", { filter });
},
...
async getSession(option: GetSessionOption) : Promise<Session | null> {
...
async getConfig(kind: ConfigKind) : Promise<Config> {
    return await TAURI_INVOKE("get_config", { kind });
},
...
export type Config = { type: "general"; data: ConfigDataGeneral } | { type: "profile"; data: ConfigDataProfile }
